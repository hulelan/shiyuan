...
要一层浅淡就用 `color-mix()` 从主色兑。备用色板在 `css/palettes.css`，
六套：朱砂纸本（默认）、素宣、夜读、青绿、茶褐、青花。

`palettes.html` 是样张页：顶上六套并排，往下是每套的完整大样（首页版式、卡片、
筛选条、各种状态）。点「全站试用」把选择存进 `localStorage`，回站上整站都跟着换。

定下来一套之后：把它的值抄进 `style.css` 的 `:root`，
删掉 `css/palettes.css`、`palettes.html` 与 `index.html` 头部那段读 `localStorage` 的脚本。
