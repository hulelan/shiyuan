...
原先是「一整行拼音」摞在「一整行汉字」上面，两块各自折行。
绝句只有五七个字，一行放得下，看着没问题；可散文一段一百多字，
两块一折就彻底错开 —— 上面十行罗马字，下面十行汉字，谁也不知道哪个音配哪个字。

改用 `<ruby>` / `<rt>`：注音焊在字上，怎么折行都跟着走。
生成拼音时用的是 `errors=lambda x: list(x)`，标点也各占一个位置，
所以拼音与汉字严格一一对应（全库核对过）。前端还留了一道保险：
某行的拼音个数与字数对不上就整行不注 —— 错位的注音比没有还坏。

标点头上不注音（在「。」上再印一个「。」没意义）。
`rt` 左右各留 `.22em`：注文比汉字宽时，相邻两个音会挤成一个词
（qīngfēng、zhōuyóu），这点内边距算进注文宽度，浏览器排版时自然把字推开。

散体默认收起注音（`.pd-poem.no-pinyin rt { display: none }`）——
一段一百三十字，整段注音铺上去篇幅翻倍，想读文章的人先被挡住。

## 行内笺注

难字底下一道虚线，点开就是释义 —— 像莎剧的笺注本，不必为一个词滚到页尾。

数据早就有了：编辑层的 `notes` 是 `[{term, explain}]`，`term` 多半是原文里
一字不差的一段（全库 13716 条里 94.4% 能在正文中定位）。从前它们只堆在
「注　释」那一栏，读到"冯虚御风"想知道意思，得先记住这四个字再往下翻。

前端把 `term` 在正文里定位，包成可点的 span，三条规矩：

- **长的先匹配** —— 有「明月之诗」就不该只标出「明月」
- **每条注只标第一次出现处** —— 通篇每个「之」都画线，等于没画
- **标过的字不再重叠标注**

定不了位的（约 5%：并列词条、篇名之类）照旧留在「注　释」栏里，一条不丢；
已经落进正文的那些在栏里标一个点，省得读者以为漏了。

**注释密度随篇幅走**（`enrich_glm.py` 的 `notes_quota()`）。原先一律"3-6 条"，
那是照绝句定的：二十个字挑三五个难点正合适。可《前赤壁赋》六百四十五字，
五条注等于没注 —— 而这些注现在是页面上的笺，密度不够就形同虚设。
按每 25 字一条估、上下封顶：绝句 3-6 条，《前赤壁赋》24-40 条（实得 43 条，
42 条能在正文定位）。

提示词里还加了一句硬要求：**term 必须是原文里一字不差的连续片段**，
不许写成"甲/乙"、"甲、乙"这种并列 —— 那是从前那 5% 对不上的主因。

诗与赋一视同仁：有 `notes` 就有笺注，没有就是干净的正文，不必分别处理。
